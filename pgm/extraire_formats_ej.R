...
# standard
f <- "etalab_cs1100501.pdf"

type <- stringr::str_extract(f, 'cs[0-9]{7}')

# Lire le pdf
u <- pdftools::pdf_text(file.path('data_origine', f))
...
v <- v %>% 
  ...
  tidyr::fill(start) %>% 
  filter(!grepl('etalab_cs|Adresse|gouv', v), start, v != "")
v <- pull(v,v)
...
xml2::xml_find_all(w, ".//xs:element") %>% 
  ...
  filter(!is.na(ref)) %>% 
  filter(ref %in% c('structureej', 'structureet', 'geolocalisation')) -> i
...
rangs <- u %>% paste0(., collapse = "\n") %>% 
  ...
  as.data.frame() %>% 
  filter(stringr::str_detect(V2, '^([A-Za-z]+$)')) %>% 
  mutate(libelle = as.character(V1)) %>% 
  select(-V1)
...
h <- right_join(g, rangs, by = c('name' = 'V2')) %>% 
